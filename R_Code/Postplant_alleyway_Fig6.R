...
SoilHealData_Clean_QCMatrix<- SoilHealthData_Clean_AnalysisDat %>%
  dplyr::select(SoilDepthRange, variety, 
                ...
                EC,
                bd_g.cm3_soil, bd_g.cm3_rock) 
ggpairs(SoilHealData_Clean_QCMatrix, columns =  2:12,
        ggplot2::aes(colour = SoilDepthRange,
                     alpha=0.7)) 

SoilPLFAData_Clean_QCMatrix<- SoilPLFAData_Clean_AnalysisDat %>%
  dplyr::select(SoilDepthRange, variety, 
                TotalPLFA_mass, TotalPLFA.nmol, TotalPLFA_count)
ggpairs(SoilPLFAData_Clean_QCMatrix, columns =  2:3,
        ggplot2::aes(colour = SoilDepthRange,
                     alpha=0.7)) 



#### ALLEYWAY Data frames for "Fall 2023" Post plant - Soil and PLFA #### 
SoilHealthData_Clean_FieldPostPlantDat_A<- SoilHealthData_Clean_AnalysisDat %>% 
...
head(SoilHealthData_Clean_FieldPostPlantDat_A)
#View(SoilHealthData_Clean_FieldPostPlantDat_A)

SoilPLFAData_Clean_FieldPostPlantDat_A<- SoilPLFAData_Clean_AnalysisDat %>% 
  filter(GrowthCycle == "Fall_2023") %>%
  filter(sampleLocation == "Alleyway") %>%
  mutate(SoilDepthRange = recode(SoilDepthRange, 
                                 "0_15"="0-15",
                                 ...
                                 "60_90"="60-90"),
         SoilDepthRange = factor(SoilDepthRange, levels = c("60-90", "30-60", "15-30", "0-15")))
head(SoilPLFAData_Clean_FieldPostPlantDat_A)
#View(SoilPLFAData_Clean_FieldPostPlantDat_A)

##### Summary of post plant data ALLEYWAY #####
SoilHealthData_Clean_FieldPostPlantDat_A_sum<- SoilHealthData_Clean_FieldPostPlantDat_A %>%
...
xtabs( ~ fact_replicateNo+SoilDepthRange+variety,
       data = SoilHealthData_Clean_FieldPostPlantDat_A_sum)


#View(SoilHealthData_Clean_FieldPostPlantDat_A_sum)
#head(SoilHealthData_Clean_FieldPostPlantDat_A_sum)

SoilPLFAData_Clean_FieldPostPlantDat_A_sum<- SoilPLFAData_Clean_FieldPostPlantDat_A %>%
  dplyr::select(fact_replicateNo,SoilDepthRange, variety,
                TotalPLFA_mass, TotalPLFA.nmol, TotalPLFA_count) %>%
  group_by(fact_replicateNo,SoilDepthRange, variety) %>%
  summarise(across(where(is.numeric), ~ mean(.x, na.rm = TRUE)))

#View(SoilPLFAData_Clean_FieldPostPlantDat_A_sum)

### Raw stats for summary data ###
raw_stats_long_alley <- SoilHealthData_Clean_FieldPostPlantDat_A_sum %>%
  pivot_longer(
...
print(raw_stats_long_alley)

raw_stats_table_alley <- raw_stats_long_alley %>%
  mutate(
    mean = round(mean, 3),
    sd   = round(sd, 3)
  ) %>%
...
library(writexl)
write_xlsx(raw_stats_table_alley,
           "SoilHealth_RawStats_PostPlant_alley.xlsx")

### Raw stats for PLFA under-vine summary data ###
raw_stats_longPLFA_alley <- SoilPLFAData_Clean_FieldPostPlantDat_A_sum %>%
  pivot_longer(
    cols = where(is.numeric),
    names_to = "parameter",
    values_to = "value"
...
print(raw_stats_longPLFA_alley)

raw_stats_tablePLFA_alley <- raw_stats_longPLFA_alley %>%
  mutate(
    mean = round(mean, 3),
    sd   = round(sd, 3)
  ) %>%
...
print(raw_stats_tablePLFA_alley)
...
print(raw_stats_tablePLFA_alley)
library(writexl)
write_xlsx(raw_stats_tablePLFA_alley,
           "SoilHealth_RawStats_PostPlant_PLFA_alley.xlsx")

## TC with variety using SD

# --- Model and CLD extraction  ---
SoilHealthData_sum_totalC_percent_lm1<- lmer(totalC_percent ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
raw_stats <- SoilHealthData_Clean_FieldPostPlantDat_A_sum %>%
...
SoilHealthData_sum_inorganicC_percent_lm1<- lmer(inorganicC_percent ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_TOC_percent_lm1<- lmer(TOC_percent ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_poxC_mg.kg_AddZero_lm1<- lmer(poxC_mg.kg_AddZero ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_molMnred.kg_lm1<- lmer(molMnred.kg ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
# #minC with variety 
SoilHealthData_sum_minC_lm1<- lmer(minC ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_mineralizableNitrogen_AddZero_lm1<- lmer(mineralizableNitrogen_AddZero ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_PotentiallyMinNitrate_lm1<- lmer(PotentiallyMinNitrate ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_PotentiallyMinAmmonium_lm1<- lmer(PotentiallyMinAmmonium ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_olsenP_mg.kg_lm1<- lmer(olsenP_mg.kg ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
raw_sd_data <- SoilHealthData_Clean_FieldPostPlantDat_A_sum %>%
  dplyr::group_by(SoilDepthRange, variety) %>%
  dplyr::summarise(
...
SoilHealthData_sum_pH_Hconc_lm1<- lmer(pH_Hconc ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_bd_g.cm3_soil_lm1<- lmer(bd_g.cm3_soil ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
# CEC with variety 
SoilHealthData_sum_CEC_lm1<- lmer(CEC ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
# # EC with variety 
SoilHealthData_sum_EC_lm1<- lmer(EC ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_ACE_protein_lm1<- lmer(ACE_protein ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilHealthData_Clean_FieldPostPlantDat_A_sum)
...
SoilHealthData_sum_TotalPLFA.nmol_lm1<- lmer(TotalPLFA.nmol ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilPLFAData_Clean_FieldPostPlantDat_A_sum)
...
# --- 1. Calculate Raw Stats (for SD) ---
raw_sd_data <- SoilPLFAData_Clean_FieldPostPlantDat_A_sum %>%
  dplyr::group_by(SoilDepthRange, variety) %>%
  dplyr::summarise(
...
# Define values for the new axis
new_min <- 0.0
new_max <- 60.0 
...
SoilHealthData_sum_TotalPLFA_count_lm1<- lmer(TotalPLFA_count ~ SoilDepthRange * variety + (1|fact_replicateNo), data=SoilPLFAData_Clean_FieldPostPlantDat_A_sum)
...
plots_nt <- lapply(plotsA, strip_titles)
...
combined_plot_A_postplant <- ggarrange(
...
final_plotA <- annotate_figure(
  combined_plot_A_postplant,
...
ggsave(
  filename = "SoilHealth_Plots_A.pdf",
  plot = final_plotA,
